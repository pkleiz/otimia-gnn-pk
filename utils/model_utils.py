...
def loss_edges(y_pred_edges, y_edges, edge_cw):
    """
    y_pred_edges: B x V x V x C   (logits)
    y_edges:      B x V x V       (targets)
    edge_cw:      class weights (numpy array or tensor)

    Returns:
        loss: scalar loss
    """
    B, V, _, C = y_pred_edges.shape  # V = num_nodes

    # Convert weights to tensor
    if not torch.is_tensor(edge_cw):
        edge_cw = torch.tensor(edge_cw, dtype=y_pred_edges.dtype, device=y_pred_edges.device)

    # Compute log-probabilities
    # reshape to B x C x (V*V)
    logp = F.log_softmax(y_pred_edges, dim=3)
    logp = logp.permute(0, 3, 1, 2).reshape(B, C, V * V)

    # Flatten targets: B x (V*V)
    y_flat = y_edges.reshape(B, V * V)

    # NLLLoss expects: input = B x C x N , target = B x N
    criterion = nn.NLLLoss(weight=edge_cw, reduction='mean')

    # Compute loss per batch (averaged by PyTorch over V*V)
    loss = criterion(logp, y_flat)

    # 🔥 COMPENSA a divisão por V² → ajusta para 1/V
    loss = loss * V
